...
extension ReactionMemberReactor {
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .makeDataSource:
            ...
            profiles.forEach {
                let member = FamilyMemberProfileResponse(memberId: $0.memberId, name: $0.name, imageUrl: $0.profileImageURL, dateOfBirth: .now)
            ...
            if  profiles.count != currentState.reactionMemberIds.count {
                ...
                for _ in 0...(len - 1) {
                    let member = FamilyMemberProfileResponse(memberId: "", name: "알 수 없음", dateOfBirth: .now)
                    items.append(FamilyMemberProfileCellReactor(member, isMe: false))
                }
            }
