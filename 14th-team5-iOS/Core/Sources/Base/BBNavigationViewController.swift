...
///
/// 삐삐 스타일의 NavigationBar의 UI나 스타일을 변경해야 한다면, 직접 해당 NavigationBar에 정의되어 있는 메서드나 프로퍼티를 통해 변경할 수 있습니다. 가령, NavigationBar의 높이를 바꿔야 한다면. `setHeight(_:)` 메서드를 호출하면 됩니다.
///
